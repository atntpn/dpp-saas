...
export async function createProductWithDPP(params: { userId: string; name: string; gtin: string; category?: string; description?: string; }) {
  try {
    ...
    const dppData = {
      ...
      digitalProductPassport: {
        ...
        issuer: { "@type": "Organization", name: "DPP Pro", identifier: "FR123456789" },
        issueDate: new Date().toISOString().split("T")[0],
        validIn: ["EU"],
        environmentalFootprint: { carbonFootprint: { value: 0, unit: "kg CO2e" }, resourceEfficiency: { recyclabilityRate: 0.0 } },
    ...
    const { error: dppError } = await supabase
      ...
      .insert({ product_id: product.id, data: dppData, qr_code_url: dppUrl, status: "published", published_at: new Date().toISOString() });
